...
class LLM:
    ...
    def get_completion(self):
        ...
        news_responses = retriever.get_news(country_code=self.country_code)
        sliced_news_responses = news_responses[:2]
        print(sliced_news_responses)

        completions = asyncio.run(self._get_completions_async(sliced_news_responses)) #todo: expand slice to more articles
        return completions
